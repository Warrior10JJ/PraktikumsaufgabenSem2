...
std::vector<float> Ufo::wayPoint(const float x1,const float y1,const float x2,const float y2,const float h,const float phi) {
    float phirad = phi * (M_PI/ 180.0f);//rechnet phi in radial/bogenmaß um
    ...
    float bx = x1 + (dx / ade) * abe;  //faktor abe auf die entfernung con a zu d unter x (einheitsvektormul)
    float by = y1 + (dy / ade) * abe;  //faktor abe auf die entfernung con a zu d unter y (einheitsvektormul)
